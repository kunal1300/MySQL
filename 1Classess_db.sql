...
select count(*) as count_of_students from students; -- to change the heading

select count(student_name) as cnt from students; -- count of particular column
...
-- GROUP BY:
-- Row level Functions - will give 1 ouput for each row
-- group level - set of records and gives one ouput
-- Restriction Here we can select only those columns which are part of our group by

select min(age) as min_age from students;
select max(age) as max_age from students;
select avg(age) as avg_age from students;

-- find total  number of student of the student in every year
select grad_year, count(*) as total from students group by grad_year;

use classess;

select * from students;
select student_id,student_name from students limit 2;
select student_id,student_name from students limit 1 offset 7;
